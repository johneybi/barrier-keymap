...
keyName(KeyID id)
{
	std::string name = KeyMap::formatKey(id, 0);
...
{
	std::string name = KeyMap::formatKey(kKeyNone, mask);
	return name.empty() ? "<none>" : name;
}
...
KeyRemapper::KeyEvent::KeyEvent(Type type, KeyID id, KeyModifierMask mask,
		KeyButton button, std::int32_t count) :
	m_type(type),
	m_id(id),
	m_mask(mask),
	m_button(button),
		...
		KeyModifierMask mask, KeyButton button)
{
	...
	if (suppressedScreen != m_suppressedChords.end()) {
		...
		if (suppressed != suppressedScreen->second.end()) {
			LOG_DEBUG1("key remap suppress chord up screen=\"%s\" key=%s button=0x%04x",
		...
		KeyModifierMask mask, std::int32_t count, KeyButton button)
{
	...
	if (suppressedScreen != m_suppressedChords.end() &&
		...
		LOG_DEBUG1("key remap suppress chord repeat screen=\"%s\" key=%s button=0x%04x",
		...
		KeyEventList& events)
{
	...
	for (PendingTapMap::iterator i = pendingTaps.begin();
		...
		LOG_DEBUG1("key remap flush tap hold screen=\"%s\" key=%s->%s button=0x%04x",
			screen.c_str(), keyName(tap.m_sourceID).c_str(),
		...
		const KeyEvent& before, const KeyEvent& after)
{
	...
	LOG_DEBUG1("key remap %s screen=\"%s\" key=%s->%s mask=%s->%s maskBits=0x%04x->0x%04x button=0x%04x",
